...
import com.javaex.posdao.PaymentDAO;
import com.javaex.posdao.StaticDAO;
...
public class PosApp {
	...
	public static void main(String[] args) {
		...
		while (true) {
				...
				while (true) {
					...
					if (orderNum == 0) {
						...
						for(int i=0; i<orderList.size(); i++) {
							System.out.println(orderList.get(i).getOrderId() + "\t\t" + orderList.get(i).getMenuName() +"\t\t" + orderList.get(i).getMenuPrice() +"\t\t" + orderList.get(i).getQuantity()+ "\t\t" + orderList.get(i).getTotalPrice() +"\t\t" + orderList.get(i).getTableNumber());
						}
			...
			case 5:
				System.out.println();
			    System.out.println("■ 카테고리별 매출 통계");
			    System.out.println("------------------------------------------------");
			    System.out.println(" 카테고리명\t총 매출액");

			    CategorySalesDAO salesDAO = new CategorySalesDAO();
			    List<CategorySalesVO> salesList = salesDAO.getCategorySales();

			    for (CategorySalesVO sales : salesList) {
			        System.out.printf(" %s\t\t%,d원\n", sales.getCategoryName(), sales.getTotalSales());
			    }
